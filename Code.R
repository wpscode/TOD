...
for (i in 1:length(df_list)){
  ...
  alphas[[i]] <- purrr::map_df(levels(data1$Age), get_alpha)
}
...
for (i in 1:length(df_list)){
  ...
  get_alphas[[i]] <- purrr::map_df(levels(data1$Age), get_alpha)
}
data_processor <- function (data,name){
  names(data) <- name
  df <- do.call("rbind",data)
...
alphas_ELP[[i]] <- purrr::map_df(levels(data1$Age), get_alpha)
}
...
alphas_PT[[i]] <- purrr::map_df(levels(data1_PT$Age),get_alpha_PT)
}
